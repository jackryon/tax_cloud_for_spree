...
module Spree
	...
	class TaxCloud
		...
		def lookup tax_cloud_transaction
			puts ""
			puts "LOOKUP PARAMS:"
			puts tax_cloud_transaction.shipment_items.inspect
			puts ""

			something = client.request(:lookup) do
		...
		def lookup_params tax_cloud_transaction
			...
			default_body.merge({ 
				...
				'cartID' => tax_cloud_transaction.shipment.number,
				'cartItems' => {
					'CartItem' => tax_cloud_transaction.shipment_items.map(&:to_hash)
				},
				'origin' => origin_address(tax_cloud_transaction),
